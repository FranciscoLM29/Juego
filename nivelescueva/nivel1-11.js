...
const GRID_WIDTH = 16;  // Número de columnas
const GRID_HEIGHT = 8;  // Número de filas

export class Zona1N11 extends Phaser.Scene {
    constructor() {
        super({ key: 'nc11' });
        ...
        this.grids = [
            {
                background: 'cueva',
                ladders: [
                    { x: 2, y: 2, connectsTo: 2 },
                    { x: 21, y: 21, connectsTo: 3 }
                ],
            ...
            },
            {
                background: 'cueva',
                ladders: [
                    { x: 2, y: 2, connectsTo: 1 },
                    { x: 13, y: 5, connectsTo: 3 }
                ],
                itemPosition: { x: 20, y: 20 },
                rocks: {
                    fixed: [
            ...
            },
            {
                background: 'cueva',
                ladders: [
                    { x: 20, y: 20, connectsTo: 1 },
                    { x: 13, y: 5, connectsTo: 2 }
                ],
                itemPosition: { x: 2, y: 2 },
                rocks: {
                    fixed: [
                        { x: 0, y: 0 },
                        { x: 0, y: 1 },
    ...
    completeLevel(time) {
        ...
        const previousTime = parseFloat(localStorage.getItem('level11Time')) || Infinity;
        if (elapsedTime < previousTime) {
            localStorage.setItem('level11Time', elapsedTime);
        }
    
        // Guardar el tiempo en el localStorage con el nombre del nivel
        localStorage.setItem('level11Time', elapsedTime); // Almacenar tiempo del nivel 6
