...
export class Zona1N8 extends Phaser.Scene {
    constructor() {
        super({ key: 'nc8' });
    ...
    completeLevel(time) {
        ...
        const previousTime = parseFloat(localStorage.getItem('level8Time')) || Infinity;
        if (elapsedTime < previousTime) {
            localStorage.setItem('level8Time', elapsedTime);
        }
    
        // Guardar el tiempo en el localStorage con el nombre del nivel
        localStorage.setItem('level8Time', elapsedTime); // Almacenar tiempo del nivel 6
