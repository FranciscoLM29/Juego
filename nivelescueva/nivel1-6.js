...
const GRID_WIDTH = 12;  // Número de columnas
const GRID_HEIGHT = 8;  // Número de filas

export class Zona1N6 extends Phaser.Scene {
    constructor() {
        super({ key: 'nc6' });
    ...
    completeLevel(time) {
        ...
        const previousTime = parseFloat(localStorage.getItem('level6Time')) || Infinity;
        if (elapsedTime < previousTime) {
            localStorage.setItem('level6Time', elapsedTime);
        }
    
        // Guardar el tiempo en el localStorage con el nombre del nivel
        localStorage.setItem('level6Time', elapsedTime); // Almacenar tiempo del nivel 6
