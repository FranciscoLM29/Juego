...
export class Zona1N9 extends Phaser.Scene {
    constructor() {
        super({ key: 'nc9' });
    ...
    completeLevel(time) {
        ...
        const previousTime = parseFloat(localStorage.getItem('level9Time')) || Infinity;
        if (elapsedTime < previousTime) {
            localStorage.setItem('level9Time', elapsedTime);
        }
    
        // Guardar el tiempo en el localStorage con el nombre del nivel
        localStorage.setItem('level9Time', elapsedTime); // Almacenar tiempo del nivel 6
