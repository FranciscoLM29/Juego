...
export class Zona1N15 extends Phaser.Scene {
    constructor() {
        super({ key: 'nc15' });
        ...
        this.grids = [
            ...
            },
            {
                background: 'cueva',
                ladders: [
                    { x: 4, y: 4, connectsTo: 2 },
                    { x: 21, y: 21, connectsTo: 1 }
                ],
    ...
    completeLevel(time) {
        ...
        const previousTime = parseFloat(localStorage.getItem('level15Time')) || Infinity;
        if (elapsedTime < previousTime) {
            localStorage.setItem('level15Time', elapsedTime);
        }
    
        // Guardar el tiempo en el localStorage con el nombre del nivel
        localStorage.setItem('level15Time', elapsedTime); // Almacenar tiempo del nivel 6
