...
export class Zona1N13 extends Phaser.Scene {
    constructor() {
        super({ key: 'nc13' });
        ...
        this.grids = [
            ...
            },
            {
                background: 'cueva',
                ladders: [
                    { x: 13, y: 5, connectsTo: 1 },
                    { x: 2, y: 2, connectsTo: 3 }
                ],
                itemPosition: { x: 20, y: 20 },
                rocks: {
                    fixed: [
    ...
    completeLevel(time) {
        ...
        const previousTime = parseFloat(localStorage.getItem('level13Time')) || Infinity;
        if (elapsedTime < previousTime) {
            localStorage.setItem('level13Time', elapsedTime);
        }
    
        // Guardar el tiempo en el localStorage con el nombre del nivel
        localStorage.setItem('level13Time', elapsedTime); // Almacenar tiempo del nivel 6
