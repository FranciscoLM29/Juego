...
export class Zona1N7 extends Phaser.Scene {
    constructor() {
        super({ key: 'nc7' });
    ...
    completeLevel(time) {
        ...
        const previousTime = parseFloat(localStorage.getItem('level7Time')) || Infinity;
        if (elapsedTime < previousTime) {
            localStorage.setItem('level7Time', elapsedTime);
        }
    
        // Guardar el tiempo en el localStorage con el nombre del nivel
        localStorage.setItem('level7Time', elapsedTime); // Almacenar tiempo del nivel 6
