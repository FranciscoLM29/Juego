...
const GRID_WIDTH = 17;  // Número de columnas
const GRID_HEIGHT = 8;  // Número de filas

export class Zona1N5 extends Phaser.Scene {
    constructor() {
        super({ key: 'nc5' });
    }
    ...
    create() {
        ...
        this.itemPosition = { x: 8, y: 4 }; // Ubicación del objeto en la cuadrícula
    ...
    completeLevel(time) {
        ...
        const previousTime = parseFloat(localStorage.getItem('level5Time')) || Infinity;
        if (elapsedTime < previousTime) {
            localStorage.setItem('level5Time', elapsedTime);
        }
    
        // Guardar el tiempo en el localStorage con el nombre del nivel
        localStorage.setItem('level5Time', elapsedTime); // Almacenar tiempo del nivel 1
